function addParticipant() {
  ...
  if (!login) { alert("Нужен логин"); return; }
  if (!fileInput.files.length) { alert("Нужна фотография"); return; }
...
fetch("/admin/add_participant", { method: "POST", body: fd })
  .then(async (r) => {
    ...
    catch { return { status: "error", msg: text }; }
  })
    .then(d => {
  if (d.status === "ok") {
    ...
    if (d.code === "NO_FACE") {
      alert("Ошибка: на фото не обнаружено лицо человека.\n\n" + (d.msg || ""));
    } else {
